...
Browser.prototype.request = function(method, path, options, fn, saveHistory){
  ...
  // HTTP client
  if (!this.client) {
    this.client = createClient(this.port, this.host, server);
  }

  var req = this.client.request(method, path, headers);
  var handleResponse = function(err, res, bodyOr$) {
    if(fn) {
      if(err) {
        fn(err);
      }
  ...
  req.on('response', function(res){
    ...
    if (res.headers['set-cookie']) {
      res.headers['set-cookie'].forEach(function(cookie) {
        if(cookie) {
          self.cookieJar.add(new Cookie(cookie, req));
        }
      });
    }
    ...
    if (status >= 200 && status < 300) {
      ...
      if (~contentType.indexOf('json')) {
        ...
        res.on('end', function(){
          try {
            ...
            handleResponse(null, res, res.body);
          } catch (err) {
            handleResponse(err);
          }
        });
      ...
      res.on('end', function(){
        self.parse(buf);
        handleResponse(null, res, function(selector){
    ...
    // Redirect
      ...
      var location = res.headers.location
        , uri = url.parse(location), path = uri.pathname+uri.search,
        ssl = uri.protocol === "https:";
      // Recreate Client when changing Protocol
      if(self.client.ssl && !ssl) {
        self.client = createClient(80, self.host, server);
      }
      else if (!self.client.ssl && ssl) {
        self.client = createClient(443, self.host, server);
      }
...
function createClient(port, host, server) {
    ...
    if (port && host) {
      ...
      return http.createClient(server.__port);
    }
}
